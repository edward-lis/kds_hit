...
void MainWindow::setupMachine()
{
    ...
    stateSerialPortClose->addTransition(ui->btnCOMPortConnect, SIGNAL(clicked(bool)), stateSerialPortOpening); // при нажатии на кнопку порта - открыть
    ...
    stateFirst->addTransition(ui->btnCOMPortConnect, SIGNAL(clicked(bool)), stateSerialPortClose); // при нажатии кнопки порта - закроем его
    ...
    stateIdleCommand->addTransition(ui->btnCOMPortConnect, SIGNAL(clicked(bool)), stateSerialPortClose); // при нажатии кнопки порта - закроем его
...
void MainWindow::enterStateSerialPortClose()
{
    ...
    ui->btnCOMPortConnect->setText(tr("Открыть")); // в этом состоянии напишем такие буквы на кнопке
...
void MainWindow::enterStateSerialPortOpening()
{
    ...
    if(serialPort->openPort(ui->comboBoxCOMPort->currentText())) // открыть порт. если он открылся нормально
    {
        ...
        ui->btnCOMPortConnect->setText(tr("Закрыть")); // в этом состоянии напишем такие буквы на кнопке
